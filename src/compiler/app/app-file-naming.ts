...
export function getCoreEsmBuildPath(config: d.Config, outputTarget: d.OutputTargetWww, sourceTarget: d.SourceTarget) {
  if (sourceTarget === 'es5') {
    return pathJoin(config, getDistEsmBuildDir(config, outputTarget), `es5`, `${config.fsNamespace}.core.js`);
  }
  return pathJoin(config, getDistEsmBuildDir(config, outputTarget), `${config.fsNamespace}.core.js`);
}
...
export function getComponentsEsmBuildPath(config: d.Config, outputTarget: d.OutputTargetDist, sourceTarget: d.SourceTarget) {
  if (sourceTarget === 'es5') {
    return pathJoin(config, getDistEsmBuildDir(config, outputTarget), `es5`, `${config.fsNamespace}.components.js`);
  }
  return pathJoin(config, getDistEsmBuildDir(config, outputTarget), `${config.fsNamespace}.components.js`);
}
